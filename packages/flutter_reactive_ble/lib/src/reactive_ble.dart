...
class FlutterReactiveBle {
  ...
  Future<Characteristic> resolveSingle(QualifiedCharacteristic characteristic) async {
    ...
    if (chars.length > 1){
      final singleChar = chars.firstWhere((char) => 
        characteristic.handle == null || char._instanceId == characteristic.handle.toString()
      );
...
class Characteristic {
  ...
  // Добавляем геттер для handle
  int get handle => int.tryParse(_instanceId) ?? -1;
